...
func TestMemoryEngine_Basic(t *testing.T) {
...
func TestMemoryEngine_Services(t *testing.T) {
	engine := NewMemoryEngine()
...
func TestMemoryEngine_BatchOperations(t *testing.T) {
	engine := NewMemoryEngine()
...
func TestMemoryEngine_BackupRestore(t *testing.T) {
	tempDir := t.TempDir()
	...
	// Verify restored data (simplified - just check if keys exist)
	keys, err := newEngine.List("")
	if err != nil {
		t.Fatalf("Failed to list keys: %v", err)
	}

	if len(keys) == 0 {
		t.Error("No keys found after restore")
	}
}

func TestMemoryEngine_Transactions(t *testing.T) {
	engine := NewMemoryEngine()
